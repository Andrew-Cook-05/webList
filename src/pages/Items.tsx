...
export default function Items() {
  ...
  async function deleteSelectedItems() {
    ...
    setItemList(prev => prev.filter(item => !idsToDelete.includes(item.id)));
    setSelectedIds([]);

    try {
      await deleteDbItems(userId, idsToDelete);
    } catch (error) {
      console.error(error);
      alert("Delete failed. " + error);
    }
  ...
  function toggleAllIds() {
    if (selectedIds.length === itemList.length) {
      ...
      setSelectedIds(itemList.map(item => item.id));
    }
  }
  ...
  return (
    <PageWrap>
      ...
      <div className={style["table-box"]}>
        ...
        <button className="button danger-button" disabled={selectedIds.length === 0} onClick={() => setIsDeleteOpen(true)}>Delete Selected Items</button>
      </div>
        <ItemTable items={itemList} sortBy={sortBy} sortDirection={sortDirection} onSort={handleSort} setDeleteTarget={setDeleteTarget} 
        updateStatus={updateStatus} selectedIds={selectedIds} toggleSelectedId={toggleSelectedId} toggleAllIds={toggleAllIds}/>
      </div>
      {isAddOpen && (
